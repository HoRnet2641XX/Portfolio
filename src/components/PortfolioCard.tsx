...
export function PortfolioCard({ item, index }: PortfolioCardProps) {
  ...
  return (
    <motion.article
      ...
      transition={{ duration: 0.4, delay: index * 0.04, ease: ease.out }}
      className={cn(
    ...
    >
      <CardImage item={item} isFeatured={isFeatured} />
      <CardBody item={item} isFeatured={isFeatured} />
...
function CardActions({ item }: { item: PortfolioItem }) {
    ...
    <div className="flex items-center gap-[6px] pt-[10px] border-t border-border-subtle mt-auto">
      {item.links?.live && (
        <a
          ...
          className="flex items-center gap-[4px] px-[10px] py-[5px] rounded-sm text-[10px] font-pixel bg-brand-subtle text-brand border border-border-brand hover:bg-brand hover:text-white transition-all duration-micro"
        >
      ...
      {item.links?.github && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-[4px] px-[10px] py-[5px] rounded-sm text-[10px] font-pixel bg-surface-base text-content-tertiary border border-border hover:text-content-primary hover:border-content-muted transition-all duration-micro"
        >
      ...
      {item.file && (
        <a
          href={item.file.path}
          download
          className="flex items-center gap-[4px] px-[10px] py-[5px] rounded-sm text-[10px] font-pixel bg-surface-base text-content-tertiary border border-border hover:text-brand hover:border-border-brand transition-all duration-micro"
        >
